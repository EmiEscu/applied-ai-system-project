...
#     (200 for tempo so raw BPM doesn't dominate; 1.0 for already-0-to-1 features).
#   - reason_threshold: per-feature closeness (1 - |diff|/norm) at/above which
#     the feature is named in the human-readable explanation. Does NOT affect
#     the score — purely for transparency.
...
def score_song(song: Song, user: UserProfile) -> Tuple[float, str]:
    ...
    space and ignores magnitude — so for non-negative features in [0,1], a
    user vector of (1,1,1,1) looks ~95–100% similar to almost any positive
    song vector. Per-feature absolute-difference respects magnitude (0.5 is
    genuinely "halfway" to 1.0), which is what we want for interpretable
    audio features. Cosine *is* used inside MMR for song-vs-song duplicate
    ...
    for u_key, s_key in _CATEGORICAL_FEATURES:
        ...
        if u_val and getattr(song, s_key) == u_val:
            weight = WEIGHTS[u_key]
            score += weight
            reasons.append(f"{u_key} match ({u_val}) (+{weight})")

    for u_key, s_key, norm, threshold, unit in _NUMERIC_FEATURES:
        u_val = getattr(user, u_key)
        if u_val is None:
            continue
        s_val = getattr(song, s_key)
        sim = 1.0 - abs(s_val / norm - u_val / norm)
        contribution = WEIGHTS[u_key] * sim
        score += contribution
        if sim >= threshold:
            reasons.append(
                f"{u_key} close ({s_val}{unit}) (+{contribution:.2f})"
            )
